...
class Application_Service_ImageUploader {
    ...
    public function upload() {
        ...
        if(!$uploader->receive()) {
            $this->errors = $uploader->getMessages();
            return false;
        }
        ...
        $image = $this->createImageModel($width, $height, Zend_Auth::getInstance()->getIdentity()->getUserID());
 
        copy($filename, $fullImagePath . DIRECTORY_SEPARATOR . $image->getImageID().'.jpg');
        unlink($filename);
        $this->resize($image);
	...
	public function resize(Application_Model_Image $image) {
		$fullURL = Zend_Registry::get('config')->paths->images->full . DIRECTORY_SEPARATOR . $image->getImageID() . '.jpg';
        $largeURL = Zend_Registry::get('config')->paths->images->large . DIRECTORY_SEPARATOR . $image->getImageID() . '.jpg';	
	...
	public function crop(Application_Model_Image $image, array $data) {
        $smallPath = Zend_Registry::get('config')->paths->images->small . DIRECTORY_SEPARATOR . $image->getImageID() . '.jpg';
        $mediumPath = Zend_Registry::get('config')->paths->images->medium . DIRECTORY_SEPARATOR . $image->getImageID() . '.jpg';
		$largePath = Zend_Registry::get('config')->paths->images->large . DIRECTORY_SEPARATOR . $image->getImageID() . '.jpg'; 
