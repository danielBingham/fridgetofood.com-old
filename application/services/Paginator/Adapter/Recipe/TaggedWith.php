<?php
class Application_Service_Paginator_Adapter_Recipe_TaggedWith
	extends Application_Service_Paginator_Adapter_Recipe_Abstract {
		
	public function __construct(Application_Model_Tag $tag, $order) {
		...
		)->where('id in (select recipe_id from recipe_tags where tag_id=?)', $tag->getTagID());
